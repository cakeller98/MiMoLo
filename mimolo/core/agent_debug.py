...
def _open_tail_window_windows(stderr_log_path: str) -> subprocess.Popen[Any] | None:
    ...
    try:
        ...
        return subprocess.Popen(tail_cmd, creationflags=creationflags)
...
def _open_tail_window_macos(stderr_log_path: str) -> subprocess.Popen[Any] | None:
...
def _open_tail_window_linux(stderr_log_path: str) -> subprocess.Popen[Any] | None:
    try:
        return subprocess.Popen(["xterm", "-e", "tail", "-f", stderr_log_path])
    except FileNotFoundError:
...
def open_tail_window(stderr_log_path: str) -> subprocess.Popen[Any] | None:
    """Open a separate terminal window to tail a log file."""
    if os.name == "nt":
        return _open_tail_window_windows(stderr_log_path)

    elif sys.platform == "darwin":
        return _open_tail_window_macos(stderr_log_path)

    else:
        # Assume Linux/Unix if not Windows or macOS.
        return _open_tail_window_linux(stderr_log_path)
